...
#' maxquant.cleanup()
#' 
maxquant.cleanup<- function(df, kinases){
  ...
  df.4<-df.3 %>% 
    rename_with(~deframe(sample)[.x], .cols = sample$Sample.ID)%>%  select(Gene, Group, any_of(sample$Treatment))
  ...
  imputed_df.2<- inner_join(sample, imputed_df.2, by = "Treatment")
